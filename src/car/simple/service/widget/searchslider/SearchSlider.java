package car.simple.service.widget.searchslider;
...
/**
 * Created by r.savuschuk on 10/2/2014.
 */
public class SearchSlider extends ViewGroup {
	 ...
	 */
	public SearchSlider(Context context, AttributeSet attrs) {
	 ...
	 */
	public SearchSlider(Context context, AttributeSet attrs, int defStyle) {
		super(context, attrs, defStyle);
		TypedArray a = context.obtainStyledAttributes(attrs, R.styleable.SearchSlider, defStyle, 0);

		int orientation = a.getInt(R.styleable.SearchSlider_direction, ORIENTATION_BTT);
		mVertical = (orientation == ORIENTATION_BTT || orientation == ORIENTATION_TTB);
		mBottomOffset = (int) a.getDimension(R.styleable.SearchSlider_bottomOffset, 0.0f);
		mTopOffset = (int) a.getDimension(R.styleable.SearchSlider_topOffset, 0.0f);
		mAllowSingleTap = a.getBoolean(R.styleable.SearchSlider_allowSingleTap, true);
		mAnimateOnClick = a.getBoolean(R.styleable.SearchSlider_animateOnClick, true);
		mInvert = (orientation == ORIENTATION_TTB || orientation == ORIENTATION_LTR);

		int handleId = a.getResourceId(R.styleable.SearchSlider_handle, 0);
		if (handleId == 0) {
			throw new IllegalArgumentException("The handle attribute is required and must refer " + "to a valid child.");
		}

		int contentId = a.getResourceId(R.styleable.SearchSlider_content, 0);
		if (contentId == 0) {
	...
	private void calculateHandleTouchArea(View handle) {
		// TODO calculate area
	}
	...
	private void initTouchRegions() {
		int xLeft = mHandle.getWidth() / 2 - 30;
		int yTop = 0;
		int xRight = mHandle.getWidth() / 2 + 30;
		int yBottom = 30;
		mFrame = new Rect(xLeft, yTop, xRight, yBottom);
	    /*touchRegionUp.set(rect);
        xLeft = getWidth() / 2 - REGION_EDGE / 2;
        yTop = getHeight() - REGION_EDGE;
        xRight = getWidth() / 2 + REGION_EDGE / 2;
        yBottom = getHeight();
        Rect rect1 = new Rect(xLeft, yTop, xRight, yBottom);
        touchRegionDown.set(rect1);*/
	}
	...
	protected void onLayout(boolean changed, int l, int t, int r, int b) {
		...
		WLog.d(LOG_TAG, "handleHeight: " + handleHeight);
		...
		if (mVertical) {
			...
			if (mInvert) {
				WLog.d(LOG_TAG, "content.layout(1)");
		...
		if (mTracking) {
			...
			switch (action) {
				...
				case MotionEvent.ACTION_CANCEL: {
					...
					if (Math.abs(velocity) < mMaximumTapVelocity*3) {//important fix (search slider react on touch)
						boolean c1;
						boolean c2;
						boolean c3;
						boolean c4;
						...
						WLog.d(LOG_TAG, "ACTION_UP: " + "c1: " + c1 + ", c2: " + c2 + ", c3: " + c3 + ", c4: " + c4);
	...
	private void performFling(int position, float velocity, boolean always) {
		...
		if (mExpanded) {
			...
			WLog.d(LOG_TAG, "position: " + position + ", velocity: " + velocity + ", mMaximumMajorVelocity: " + mMaximumMajorVelocity);
			...
			WLog.d(LOG_TAG, "EXPANDED. c1: " + c1 + ", c2: " + c2 + ", c3: " + c3);
			if (always || (c1 || (c2 && c3))) {
			...
			WLog.d(LOG_TAG, "COLLAPSED. position: " + position + ", velocity: " + velocity + ", mMaximumMajorVelocity: " + mMaximumMajorVelocity);
			WLog.d(LOG_TAG, "COLLAPSED. always: " + always + ", c1: " + c1 + ", c2: " + c2 + ", c3: " + c3);
	...
	private void prepareContent() {
		...
		if (content.isLayoutRequested()) {
			...
			if (mVertical) {
				...
				WLog.d(LOG_TAG, "content.layout(2)");
	...
	/**
	 * Set content to list that is located in SlidingDrawer
	 *
	 * @param adapter  - adapter with content
	 * @param listener - AdapterView.OnItemClickListener
	 */
	//public void setContentToList(BaseAdapter adapter, AdapterView.OnItemClickListener listener) {

	//			ListView list = (ListView) findViewById(R.id.search_layout);
	...
	//}
		...
		/*public void addViewTransparent(View view){
			mTransparentBackground = view;
		}*/
}
